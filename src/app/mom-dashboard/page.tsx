            ...
            {([
              ...
              <button
                ...
                style={{
                  padding: '7px 18px',
                  borderRadius: 20,
                  border: activeTab === key ? 'none' : '1px solid rgba(201,124,138,.35)',
                  background: activeTab === key ? '#C97C8A' : 'transparent',
                  color: activeTab === key ? '#fff' : '#C97C8A',
                  fontSize: 13,
                    ...
                    return (
                      <div key={group.months} style={{ marginBottom: 10 }}>
                        ...
                        {allItems.map(item => {
                          ...
                          return (
                            <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 0', borderBottom: '0.5px solid rgba(201,124,138,.05)' }}>
                              ...
                              <div style={{ flex: 1, minWidth: 0 }}>
                                ...
                                {vendorName && <div style={{ fontSize: 10, color: '#C97C8A', fontWeight: 500 }}>{vendorName}</div>}
                              </div>
                              <div style={{ display: 'flex', gap: 3, flexShrink: 0 }}>
                                {!vendorName && addVendor !== item.id && (
                                  <button onClick={() => setAddVendor(item.id)}
                                    style={{ fontSize: 10, color: '#C97C8A', background: 'rgba(201,124,138,.08)', border: 'none', borderRadius: 20, padding: '2px 7px', cursor: 'pointer' }}>+ Add</button>
                                )}
                                {addVendor === item.id && (
                ...
                {VENDOR_CATEGORIES.map((cat, i) => {
                  ...
                  return (
                    <div key={cat.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 0', borderBottom: i < VENDOR_CATEGORIES.length - 1 ? '0.5px solid rgba(201,124,138,.06)' : 'none' }}>
                      <div style={{ fontSize: 10, fontWeight: 600, color: '#C97C8A', textTransform: 'uppercase', letterSpacing: '.4px', width: 90, flexShrink: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{cat.label}</div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16, minWidth: 0 }}>
              ...
              <div style={{ background: '#fff', border: '0.5px solid rgba(201,124,138,.18)', borderRadius: 14, padding: '16px 18px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8, marginBottom: 8 }}>
                  <div className="font-serif" style={{ fontSize: 28, color: '#1a0a0f', lineHeight: 1 }}>{fmt(totalPaid)}</div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                  {VENDOR_CATEGORIES.map(cat => {
                    ...
                    const vendorName = item?.vendor_name_override || (item?.vendors as any)?.business_name || ''
                    return (
                      <div key={cat.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '4px 0', borderBottom: '0.5px solid rgba(201,124,138,.06)', gap: 8 }}>
                        <span style={{ fontSize: 12, color: '#555', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{cat.label}</span>
                        <span style={{ fontSize: 12, fontWeight: 500, color: vendorName ? '#1a0a0f' : '#ccc', flexShrink: 0 }}>{vendorName || '—'}</span>
                      </div>
                    )
                  })}
                  {budget > 0 && (
        ...
        {activeTab === 'compare' && (
          <div>
            <div style={{ marginBottom: 16 }}>
              ...
              <p style={{ fontSize: 13, color: '#7a5c65', marginTop: 4 }}>Browse vendors by category and add them to your checklist</p>
            </div>
            {VENDOR_CATEGORIES.map(cat => {
              const item       = checklist.find(cl => cl.item_name?.toLowerCase().includes(cat.label.toLowerCase()))
              ...
              const vendorName = item?.vendor_name_override || (item?.vendors as any)?.business_name || ''
              return (
                <div key={cat.id} style={{ background: '#fff', border: `0.5px solid ${isBooked ? 'rgba(26,122,74,.2)' : 'rgba(201,124,138,.18)'}`, borderRadius: 12, marginBottom: 8, padding: '13px 18px', display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{ width: 19, height: 19, borderRadius: '50%', border: `2px solid ${isBooked ? '#1a7a4a' : '#C97C8A'}`, background: isBooked ? '#1a7a4a' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
