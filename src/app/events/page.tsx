...
export default function EventsPage() {
  ...
  return (
    <>
      <Nav />
      <div style={{ background: '#1a0a0f', padding: '40px 24px 32px' }}>
        <div style={{ maxWidth: 800, margin: '0 auto', textAlign: 'center' }}>
          ...
          <h1 className="font-serif" style={{ fontSize: 'clamp(28px,8vw,42px)', color: '#fff', marginBottom: 10 }}>Houston Quinceañera Events</h1>
          <p style={{ fontSize: 14, color: 'rgba(250,216,233,.6)', lineHeight: 1.6 }}>Expos, open houses, and vendor events happening near you</p>
        </div>
      </div>
      <div style={{ background: '#fff', borderBottom: '0.5px solid rgba(201,124,138,.15)', padding: '12px 16px', overflowX: 'auto', display: 'flex', gap: 8, scrollbarWidth: 'none' as const }}>
        {['All', 'Free', 'Expos', 'Open Houses', 'Coming Soon'].map((f) => (
          <button key={f} onClick={() => setActiveFilter(f)}
            style={{ background: activeFilter === f ? '#C97C8A' : 'transparent', color: activeFilter === f ? '#fff' : '#7a5c65', border: '0.5px solid rgba(201,124,138,.3)', borderRadius: 20, padding: '7px 16px', fontSize: 13, cursor: 'pointer', whiteSpace: 'nowrap' as const, flexShrink: 0 }}>{f}</button>
        ))}
      </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(300px,1fr))', gap: 20 }}>
          {EVENTS.map((ev, i) => (
            <div key={i} style={{ background: '#fff', border: '0.5px solid rgba(201,124,138,.15)', borderRadius: 16, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
              <div style={{ height: 160, background: ev.bg, position: 'relative', display: 'flex', alignItems: 'flex-start', padding: 14, gap: 8, flexWrap: 'wrap' as const }}>
                {ev.free && <div style={{ background: '#1a7a4a', color: '#fff', fontSize: 10, fontWeight: 700, padding: '4px 10px', borderRadius: 20, textTransform: 'uppercase' as const, height: 'fit-content' }}>Free Entry</div>}
                {!ev.free && <div style={{ background: '#1a0a0f', color: '#FAD8E9', fontSize: 10, fontWeight: 700, padding: '4px 10px', borderRadius: 20, textTransform: 'uppercase' as const, height: 'fit-content' }}>Paid Entry</div>}
                {ev.status === 'coming_soon' && <div style={{ background: '#C9A040', color: '#1a0a0f', fontSize: 10, fontWeight: 700, padding: '4px 10px', borderRadius: 20, textTransform: 'uppercase' as const, height: 'fit-content' }}>Coming Soon</div>}
              ...
              <div style={{ padding: '16px 18px', flex: 1, display: 'flex', flexDirection: 'column' }}>
                <span style={{ background: 'rgba(201,124,138,.1)', color: '#C97C8A', fontSize: 10, fontWeight: 600, padding: '3px 8px', borderRadius: 20, textTransform: 'uppercase' as const, display: 'inline-block', marginBottom: 8 }}>{TYPE_LABELS[ev.type]}</span>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto 48px', padding: '0 16px' }}>
        <div style={{ background: '#FDF6F0', border: '0.5px solid rgba(201,124,138,.2)', borderRadius: 16, padding: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap' as const, gap: 16 }}>
          <div>
