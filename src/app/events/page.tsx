...
const EVENTS = [
  ...
  { title: 'My 15 Expo — Houston', type: 'expo', date: 'Sun, Jan 18, 2026 · 12–5 PM', location: 'NRG Center Exhibit Hall A1-A3, Houston TX', description: '22 years running — the country's largest quinceañera trade show. Get access to the best vendors, planning resources, and inspiration all in one place.', free: true, status: 'upcoming', ctaLabel: 'Learn More', ctaHref: '#', bg: 'linear-gradient(155deg,#201525 0%,#604070 55%,#9060a0 100%)' },
    ...
    <>
      ...
      <div style={{ background: '#1a0a0f', padding: '40px 28px 32px' }}>
        <div style={{ maxWidth: 800, margin: '0 auto', textAlign: 'center' }}>
          ...
          <h1 className="font-serif" style={{ fontSize: 42, color: '#fff', marginBottom: 10 }}>
            Houston Quinceañera <em style={{ color: '#FAD8E9' }}>Events</em>
          </h1>
          <p style={{ fontSize: 14, color: 'rgba(250,216,233,.6)', lineHeight: 1.6 }}>Expos, open houses, and vendor events happening near you — all in one place</p>
        </div>
      </div>

      {/* Filter strip */}
      <div style={{ background: '#fff', borderBottom: '0.5px solid rgba(201,124,138,.15)', padding: '12px 28px', display: 'flex', gap: 10, overflowX: 'auto' }}>
        {['All','Free','Expos','Open Houses','Coming Soon'].map((f, i) => (
          <button key={f} style={{ background: i === 0 ? '#C97C8A' : 'transparent', color: i === 0 ? '#fff' : '#7a5c65', border: '0.5px solid rgba(201,124,138,.3)', borderRadius: 20, padding: '7px 16px', fontSize: 13, cursor: 'pointer', whiteSpace: 'nowrap' }}>{f}</button>
        ))}
      </div>

      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '40px 28px', display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 20 }}>
        {EVENTS.map((ev, i) => (
          <div key={i} style={{ background: '#fff', border: `0.5px solid ${ev.myq ? 'rgba(201,160,64,.4)' : 'rgba(201,124,138,.15)'}`, borderRadius: 16, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            <div style={{ height: 180, background: ev.bg, position: 'relative', display: 'flex', alignItems: 'flex-start', padding: 14, gap: 8 }}>
              {ev.free && <div style={{ background: '#1a7a4a', color: '#fff', fontSize: 10, fontWeight: 700, padding: '4px 10px', borderRadius: 20, textTransform: 'uppercase', letterSpacing: .5 }}>Free Entry</div>}
              {!ev.free && <div style={{ background: '#1a0a0f', color: '#FAD8E9', fontSize: 10, fontWeight: 700, padding: '4px 10px', borderRadius: 20, textTransform: 'uppercase', letterSpacing: .5 }}>Paid Entry</div>}
              {ev.status === 'coming_soon' && <div style={{ background: '#C9A040', color: '#1a0a0f', fontSize: 10, fontWeight: 700, padding: '4px 10px', borderRadius: 20, textTransform: 'uppercase', letterSpacing: .5 }}>Coming Soon</div>}
              {ev.myq && (
                <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, background: 'rgba(201,160,64,.9)', padding: '6px 14px', fontSize: 11, fontWeight: 600, color: '#1a0a0f' }}>
                  ★ MyQuinceAños Official Event
                </div>
              )}
            </div>
            <div style={{ padding: '16px 18px', flex: 1, display: 'flex', flexDirection: 'column' }}>
              <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
                <span style={{ background: 'rgba(201,124,138,.1)', color: '#C97C8A', fontSize: 10, fontWeight: 600, padding: '3px 8px', borderRadius: 20, textTransform: 'uppercase', letterSpacing: .5 }}>{TYPE_LABELS[ev.type]}</span>
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 12, color: '#7a5c65', marginBottom: 10 }}>
              ...
              <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                ...
                {ev.price && <span style={{ fontSize: 12, color: '#7a5c65' }}>{ev.price}</span>}
              </div>
            </div>
          </div>
        ))}
